// quiz-sell-genius/src/components/QuizQuestion.tsx
...
const QuizQuestion: React.FC<QuizQuestionProps> = ({
  ...
  const handleOptionSelect = (optionId: string) => {
    ...
    if (autoAdvance && newSelectedOptions.length > 0 && onNextClick) {
      setTimeout(() => onNextClick(), 300);
    }
  };

  const getGridColumns = () => {
    if (question.type === 'text') {
      if (isStrategicQuestion) return "grid-cols-1 gap-3 px-2";
          ...
          <>
            ...
            {!isStrategicQuestion && (
              <p className="text-xs sm:text-sm text-[#1A1818]/70 px-2 py-2 mb-4 text-center font-medium">
                Selecione {question.multiSelect} Opções
              </p>
            )}
          </>
        )}
